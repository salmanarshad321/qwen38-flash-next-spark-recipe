...
from . import model
from vllm_fp8_hybrid_modelopt import excluded_quant_config as _fp8_hybrid_excluded
...
class Qwen3_8FlashNextQSAFlashAttentionImpl(FlashAttentionImpl):
    ...
    def forward_qsa(
        ...
        qsa_sparse_paged_attention(
            ...
            token_to_req,
            output[:num_tokens],
            output_gate=output_gate[:num_tokens] if output_gate is not None else None,
        )
        return output
...
class Qwen3_8FlashNextQSAAttention(Qwen3NextAttention, AttentionLayerBase):
    ...
    def __init__(
        ...
        *,
        ...
        self.fuse_output_gate = os.environ.get("VLLM_QSA_FUSE_OUTPUT_GATE", "1") == "1"
        ...
        self.qkv_proj = QKVParallelLinear(
            ...
            bias=False,
            quant_config=_fp8_hybrid_excluded(quant_config),
    ...
    def forward(
        ...
        if current_platform.opaque_attention_op():
            torch.ops.vllm.qwen3_8_flash_next_qsa_with_output(
                ...
                value,
                attn_output,
                gate if self.fuse_output_gate else None,
                encoded_layer_name,
            )
        else:
        ...
        if gate is not None and not self.fuse_output_gate:
