...
def safetensors_weights_iterator(
    ...
    *,
    ...
    for st_file in tqdm(
        ...
        # PLE mmap consumes these lookup tables through its own mapping. Keep
        # the standard lazy iterator for these files so eager IO does not read
        # another 47.7 GiB for each target/draft model load. Every tensor key is
        # still yielded through safe_open; no weights or dtypes are changed.
        lazy_ple_mmap = (
            safetensors_load_strategy == "eager"
            and os.environ.get("VLLM_PLE_MMAP") == "1"
            and os.path.basename(st_file).startswith("model-plefp8-")
        )
        if lazy_ple_mmap:
            logger.info_once("Keeping PLE mmap checkpoint files lazy during eager loading")
        if safetensors_load_strategy == "eager" and not lazy_ple_mmap:
