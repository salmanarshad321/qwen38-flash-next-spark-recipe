...
Knobs (env):
  ...
  VLLM_PLE_MMAP_MADVISE=random  random (default, no readahead) or normal
  ...
  VLLM_PLE_ROW_CACHE_MIB=64  process-wide resident raw-row cache, including tags;
                             0 disables; does not alter table precision
...
from ple_row_cache import RowCacheBudget
...
def _madvise(mm: np.memmap, kind: str) -> None:
    """Apply best-effort access advice to the mmap behind a NumPy memmap."""
    try:
...
# Fixed resident byte budget shared by every table in this process. Checkpoint
# bytes and their precision never change; zero disables the row cache.
_ROW_CACHE_BUDGET = RowCacheBudget(
    max(0, _env_int("VLLM_PLE_ROW_CACHE_MIB", 64)) * 1024 * 1024
)
...
class MmapPleTable:
    ...
    def __init__(
        ...
        for idx, (path, offset, rows) in shards.items():
            ...
            # The PLE table is accessed as small hashed row lookups. Avoid
            # pulling adjacent pages into cache when those rows are not used.
        ...
        self.row_cache = _ROW_CACHE_BUDGET.make_cache(self.row_bytes)

    def gather(self, ids: np.ndarray, *, sorted_unique: bool = False) -> np.ndarray:
        """ids: int64 [N] global row ids -> uint8 [N, row_bytes] (a fresh array)."""
        import time as _time

        t0 = _time.perf_counter()
        try:
            return self._gather(ids, sorted_unique=sorted_unique)
        finally:
    ...
    def _gather(self, ids: np.ndarray, *, sorted_unique: bool = False) -> np.ndarray:
        ids = np.ascontiguousarray(ids, dtype=np.int64).reshape(-1)
        if ids.size == 0:
            return np.empty((0, self.row_bytes), dtype=np.uint8)
        # The embedding caller has already deduplicated IDs for GPU expansion.
        # Preserve the generic API for callers with unsorted/repeated IDs.
        if sorted_unique:
            uniq, inverse = ids, None
        else:
        ...
        rows = self.row_cache.gather(uniq, self._read_sorted_unique)
        return rows if inverse is None else rows[inverse]

    def _read_sorted_unique(self, ids: np.ndarray) -> np.ndarray:
        """Read cache misses; IDs remain sorted/unique after hit filtering."""
        ...
        if len(tasks) == 1 or ids.size <= self.fast_rows:
            for task in tasks:
                run(task)
        else:
...
class _MmapNgramEmbedding(nn.Module):
    ...
    def forward(self, ids: torch.Tensor) -> torch.Tensor:
        ...
        rows = table.gather(uniq, sorted_unique=True)  # uint8 [U, row_bytes]
        u = rows.shape[0]
...
def _stats_log() -> None:
    ...
    caches = [
        layer.ngram_embedding.table.row_cache.stats()
        for layer in _REGISTRY.values()
        if layer.ngram_embedding.table is not None
    ]
    if caches:
        hits = sum(cache["hits"] for cache in caches)
        misses = sum(cache["misses"] for cache in caches)
        logger.info(
            "PLE byte-row cache (cumulative): %d hits, %d misses, %.1f%% hit, "
            "%d evictions, %.2f MiB resident-array cap",
            hits, misses, 100.0 * hits / max(1, hits + misses),
            sum(cache["evictions"] for cache in caches),
            sum(cache["allocated_bytes"] for cache in caches) / 2**20,
        )
...
def apply(cls: type) -> None:
    ...
    def _setup_table(self) -> None:
        ...
        logger.info(
            "PLE byte-row cache: layer %d, %d rows, %.2f MiB arrays; "
            "process cap %.2f MiB",
            layer_idx, table.row_cache.capacity,
            table.row_cache.allocated_bytes / 2**20,
            _ROW_CACHE_BUDGET.byte_budget / 2**20,
        )
        logger.info(
