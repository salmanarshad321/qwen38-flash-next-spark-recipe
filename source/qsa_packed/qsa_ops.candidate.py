...
def _qsa_sparse_paged_gqa_splitk_kernel(
    ...
    if NUM_SPLITS == 1:
        if HAS_OUTPUT_GATE:
            # vLLM #55309: keep attention's BF16 rounding before the post-op.
...
def qsa_mqa_paged(
    ...
    # Isolated backport of lovablelabs/vllm#1, commit 6b4ffca2808d92f304059fcd1a81691c18633e2b.
    # Keep the historical narrow shape gate; all other shapes use the original.
    use_query_packing = (
        os.environ.get("VLLM_QSA_QUERY_PACKING", "1") == "1"
        and q.shape[0] == 511
        and q.shape[1] == 4
        and q.shape[2] == 128
        ...
        and k_cache.shape[1] == 400
        and columns == 65600
    )
    scoring_kernel = (
        _qsa_mqa_paged_packed_kernel if use_query_packing else _qsa_mqa_paged_kernel
    )
    program_rows = triton.cdiv(q.shape[0], 4) if use_query_packing else q.shape[0]
    scoring_kernel[
        (program_rows, triton.cdiv(columns, BLOCK_N * tiles_per_program))
    ](
        q,
        k_cache,
        page_table,
...
# Source: https://github.com/lovablelabs/vllm/pull/1
# Pinned commit: 6b4ffca2808d92f304059fcd1a81691c18633e2b
# Original Apache-2.0/SPDX attribution retained above. Helpers copied verbatim.
@triton.jit
def _qsa_mqa_paged_single_row(
    row,
    q_ptr,
    k_cache_ptr,
    page_table_ptr,
...
@triton.jit
def _qsa_mqa_paged_packed_kernel(
    q_ptr,
    k_cache_ptr,
    page_table_ptr,
    token_to_req_ptr,
    ...
    row_start = tl.program_id(0) * 4
    slots = tl.arange(0, 4)
    rows = row_start + slots
    row_live = rows < num_rows
    requests = tl.load(token_to_req_ptr + rows, mask=row_live, other=-1)
    first_request = tl.load(token_to_req_ptr + row_start)
    same_request = (
        tl.sum(
            (
                row_live
    ...
    if same_request:
        positions = tl.load(query_positions_ptr + rows)
        sequence_length = tl.load(sequence_lengths_ptr + first_request)
        visible = tl.minimum(
            (positions + 1) // COMPRESS_RATIO,
            sequence_length // COMPRESS_RATIO,
        )
        if tl.program_id(1) == 0:
            tl.store(visible_blocks_ptr + rows, visible)
        max_visible = tl.max(visible, axis=0)
        tile_start = tl.program_id(1) * TILES_PER_PROG
        if tile_start * BLOCK_N < max_visible:
            tile_end = tl.minimum(
                tile_start + TILES_PER_PROG, tl.cdiv(max_visible, BLOCK_N)
            )
            tile_end = tl.minimum(tile_end, tl.cdiv(num_columns, BLOCK_N))
            dims = tl.arange(0, BLOCK_D)
            heads = tl.arange(0, MAX_N)
            query = tl.load(
                q_ptr
                + (row_start + heads[None, :] // 4) * stride_q_row
            ...
            for tile in tl.range(tile_start, tile_end, num_stages=STAGES):
                ...
                physical_page = tl.load(
                    page_table_ptr
                    + first_request * stride_table_req
                    + logical_page * stride_table_page,
                    mask=live,
                    other=-1,
                )
                ...
                packed_scores = tl.dot(keys, query, out_dtype=tl.float32)
                for slot in tl.static_range(4):
                    # Keep the MMA layout: gather forced a blocked layout with
                    # a different FP32 head-add tree in the first GPU trial.
                    # Four-head-aligned masks retain adjacent head pairs.
                    scores = tl.where(
                        heads[None, :] // 4 == slot,
                        tl.maximum(packed_scores, 0.0),
                        0.0,
                    )
                    score = tl.sum(scores, axis=1) / score_divisor
                    row_visible = tl.sum(tl.where(slots == slot, visible, 0), axis=0)
                    tl.store(
                        logits_ptr + (row_start + slot) * stride_logits_row + columns,
                        tl.where(page_valid, score, -float("inf")),
                        mask=(columns < row_visible) & (columns < num_columns),
                    )
    else:
        for slot in tl.static_range(4):
            row = row_start + slot
            if row < num_rows:
                _qsa_mqa_paged_single_row(
                    row,
                    q_ptr,
                    k_cache_ptr,
                    page_table_ptr,
